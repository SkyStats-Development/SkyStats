	...
	async onCommand(username, message) {
		try {
			...
			hypixel.getPlayer(username).then(player => {
				if (!duel) {
					this.send(`/r [Duels] [${player.stats.duels.division}] ${username} Wins: ${player.stats.duels.wins} | CWS: ${player.stats.duels.winstreak} | BWS: ${player.stats.duels.bestWinstreak} | WLR: ${player.stats.duels.WLRatio}`)	
				} else {
					if (Object?.keys(player?.stats?.duels?.[duel]).includes('overall') || Object?.keys(player?.stats?.duels?.[duel]).includes('1v1')) {
						this.send(`/r [Duels] [${duel.toUpperCase() ?? 'Unknown'}] [${player.stats.duels?.[duel]?.[Object.keys(player.stats.duels[duel])[0]]?.division ?? 'Unknown'}] ${username ?? 0} Wins: ${player.stats.duels?.[duel]?.[Object.keys(player.stats.duels[duel])[0]]?.wins ?? 0} | CWS: ${player.stats.duels?.[duel]?.[Object.keys(player.stats.duels[duel])[0]]?.winstreak ?? 0} | BWS: ${player.stats.duels?.[duel]?.[Object.keys(player.stats.duels[duel])[0]]?.bestWinstreak ?? 0} | WLR: ${player.stats.duels?.[duel]?.[Object.keys(player.stats.duels[duel])[0]]?.WLRatio ?? 0}`)
					} else {
						this.send(`/r [Duels] [${duel.toUpperCase() ?? 'Unknown'}] [${player.stats.duels?.[duel]?.division ?? 'Unknown'}] ${username ?? 0} Wins: ${player.stats.duels?.[duel]?.wins ?? 0} | CWS: ${player.stats.duels?.[duel]?.winstreak ?? 0} | BWS: ${player.stats.duels?.[duel]?.bestWinstreak ?? 0} | WLR: ${player.stats.duels?.[duel]?.WLRatio ?? 0}`)
					}
				}
			}).catch(error => {this.send('/r ' + error.toString().replaceAll('[hypixel-api-reborn] ', ''))})

		} catch (error) {
			console.log(error)
			this.send('/r There is no player with the given name or this duel does not exist.')
		}
	}
}
