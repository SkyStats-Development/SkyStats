  ...
  async onCommand(username, message) {
    try {
      ...
      const guild = await hypixel.getGuild("name", guildName).catch((err) => {
        return this.send("/r This guild does not exist.");
      });

      this.send(`/r Guild ${guildName} | Tag: ${guild.tag} | Members: ${guild.members.length} | Level: ${guild.level} | Weekly GEXP: ${guild.totalWeeklyGexp}`);
    } catch (error) {
      console.log(error);
