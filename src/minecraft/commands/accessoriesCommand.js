...
class accessoriesCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      this.send(`/r ${username}'s Accessories » ${talismanCount} | Recombobulated » ${recombobulated} | Enriched » ${enrichment}`)
      await delay(690)
      this.send(`/r ${username}'s Accessories » Common - ${common} | Uncommon - ${uncommon} | Rare - ${rare} | Epic - ${epic} |  Legendary - ${legendary} | Special - ${special} | Very Special - ${verySpecial}`)
    
    } catch (error) {
      console.log(error)
      this.send(`/r [ERROR] ${error}`)
    }
  }
}
