...
class equipmentCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      if (!profile.profile.equippment_contents?.data) return this.send(`/r This player has an Inventory API off.`)
      ...
      response == '' ? this.send(`/r ${username} has no equipment equiped.`) : this.send(`/r ${username}'s Equipment » ${response}`)
    } catch (error) {
      console.log(error)
