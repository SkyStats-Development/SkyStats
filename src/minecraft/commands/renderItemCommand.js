...
class renderCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      if (!arg[0]) this.send('/r Wrong Usage: !render [name] [slot] | !render [slot]')
      if (!isNaN(Number(arg[0]))) {
        ...
        if (!isNaN(Number(arg[1]))) {
          ...
          this.send('/r Wrong Usage: !render [name] [slot] | !render [slot]')
          return
        }
      }
      
      const profile = await getLatestProfile(username);
      if (!profile.profile.inv_contents?.data) return this.send(`/r This player has an Inventory API off.`)
      ...
      if (!inventoryData[itemNumber - 1] || !Object.keys(inventoryData[itemNumber - 1] || {}).length) {this.send(`/r Player does not have an item at slot ${itemNumber}.`)}
      ...
      this.send(`/r ${username}'s item at slot ${itemNumber} » ${upload.data.link ?? 'Something went Wrong..'}`)
