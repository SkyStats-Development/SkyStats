...
class armorCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      if (!profile.profile.inv_armor?.data) return this.send(`/r This player has an Inventory API off.`)
      ...
      response == '' ? this.send(`/r ${username} has no armor equiped.`) : this.send(`/r ${username}'s armor » ${response}`)
    } catch (error) {
      console.log(error)
