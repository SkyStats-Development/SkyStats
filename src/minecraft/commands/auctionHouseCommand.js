...
class auctionHouseCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      // Could have been done better and faster using Promise.all(), I'm lazy and this works.
      ...
      for (let i = 0; i < response.auctions.length; i++) {
        if (response.auctions[i].end >= Date.now()) {
          ...
          if (!upload.data.link) this.send(`/r There was an error with Imgur, try again.`)
          string += string == '' ? upload.data.link : ' | ' + upload.data.link 
        }
        if (i == response.auctions.length-1 && string != '') {
          this.send(`/r ${username}'s Active Auctions » ${string}`); 
          break;
        }
      }
      if (string == '') this.send(`/r has no active auctions.`)
    } catch (error) {
      console.log(error)
      this.send(`/r There was an error with the command, try again.`)
    }
  }
}
