...
import { calcSkill } from '../utils/calcSkills';
...
/**
 * Get dungeon stats for a player and profile.
 */
export async function getDungeons(player: any, profile: any) {
	try {
		...
		const floors = available_floors.reduce((acc: any, floor: string) => {
		...
		return {
			selected_class: functions.capitalize(profile?.dungeons?.selected_dungeon_class ?? 'none'),
