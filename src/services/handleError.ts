...
/**
 * Handle various error types and return a Discord embed object.
 */
export function handleError(error: any) {
	if (
		error instanceof TypeError &&
		error.message.includes("Cannot read properties of undefined (reading 'cute_name')")
	) {
		console.log(error);
		return {
			color: 0xff0000,
		...
		return {
			...
			title: error.response.data.message,
			description:
				'An unknown error with searching your username has happened, please double check and try again!',
			timestamp: new Date().toISOString(),
			footer: {
				text: messages.default,
				iconURL: messages.icon,
		...
		return {
			...
			title: 'Error',
			description:
				'The Hypixel API is currently experiencing inconviniences. Please try again sometime in 3-5 minutes to days.',
			timestamp: new Date().toISOString(),
			footer: {
				text: messages.default,
				iconURL: messages.icon,
		...
		return {
			...
			title: 'Error',
			description:
				'The Hypixel API has been rate limited due to the developer key being used too much today. Please try again tomorrow.',
			timestamp: new Date().toISOString(),
			footer: {
				text: messages.default,
				iconURL: messages.icon,
		...
		return {
			...
			title: 'Error',
			description:
				'Our access to the Hypixel API has been rate limited. Please try again in a few minutes. If the error persists, please contact @axle.coffee',
			timestamp: new Date().toISOString(),
			footer: {
				text: messages.default,
				iconURL: messages.icon,
		...
		if (error.message === "Cannot destructure property 'memberData' of '(intermediate value)' as it is null.") {
			return {
				...
				title: 'Error',
				description:
					'You do not currently have an account verified with SkyStats, please link your discord account with `/verify`.',
				timestamp: new Date().toISOString(),
				footer: {
					text: messages.default,
					iconURL: messages.icon,
...
/**
 * Handle player-specific errors for Discord embeds.
 */
export async function handlePlayer(error: string) {
	if (error === 'unlinked') {
		return {
			color: 0xff0000,
			title: 'Error',
